...
| `TEST_CLUSTER_VIRTUAL_MACHINE_CLASS_NAME` | `generic` | VM class for VMs on the base cluster in `alwaysCreateNew`. If set to another DNS-label name and the class does not exist, it is created from `generic` with `spec.cpu.type: Host`, labeled `storage-e2e.deckhouse.io/auto-created=true`, and left after cleanup |
