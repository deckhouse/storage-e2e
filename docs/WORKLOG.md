...
- **Bugfix** `internal/provisioning/dvp/{provider.go,connect.go}`: fix `govet` shadow lint — reuse the function-level `err` (`=`) in `Bootstrap`/`Remove` sequential checks and rename the cleanup-closure locals to `closeErr`.
