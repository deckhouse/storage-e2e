...
- **Refactor** `internal/provisioning/dvp/provider.go`: inject `baseConnector`/`kubeOps`/`fleetFactory` (new `deps.go`, `connect.go`); first `provider_test.go` covers Bootstrap/Remove order, error short-circuit, cleanup. Behavior unchanged.
- **Update** PR #34 review fixes: document `baseConnector` nil-cleanup-on-error contract (`deps.go`); rework `provider_test.go` around a shared `recorder` (drop dead helper params, use `slices.Equal`/`slices.Contains`, `t.Parallel()`) and assert `errors.Is` wrapping for provision/teardown errors (new `TestRemoveTeardownError`); add the `internal/provisioning/` subtree to `docs/ARCHITECTURE.md` §1.1.
- **Bugfix** `internal/provisioning/dvp/{provider.go,connect.go}`: fix conflicting `govet shadow` / `gocritic sloppyReassign` lints — give the kube-step checks distinct `:=` locals (`reachErr`/`moduleErr`/`nsErr`), keep terminal `Provision`/`Teardown` checks as `:=` (no later `err` use), and rename the cleanup-closure locals to `closeErr`.
- **Add** `internal/provisioning/dvp/bootstrap.go`: embedded-template bootstrap-config rendering (`buildBootstrapParams`/`renderBootstrapConfig`/`calculateNetworkCIDR`), functional core, fully unit-tested.
- **Update** `internal/provisioning/dvp/bootstrap.go`: validate required DKP fields in `buildBootstrapParams`, parse embedded template once via `template.Must`; add branch-coverage tests; sync ARCHITECTURE.md.
- **Add** `ssh/v2.Client.Exec` returning `ExecResult` (separate `Stdout`/`Stderr` + `ExitCode`), reconnect-aware via `withConn`; test server now serves `session`/`exec`.
- **Bugfix** `internal/infrastructure/ssh/v2/exec.go`: run command exactly once — only `client.NewSession()` goes through `withConn` (retry on transient open failure); a mid-flight drop now surfaces the error instead of re-running the command and causing duplicate side effects (matches the only-open-retry contract in ARCHITECTURE §3.4.1).
- **Update** `docs/ARCHITECTURE.md` §3.4.1: document `Exec` in Public API + its only-open-retry semantics; drop the `Run` extension-point entry now realized as `Exec`.
- **Refactor** `internal/infrastructure/ssh/v2/testserver_test.go`: guard `testServer.execHandler` with `mu` via `setExecHandler`/`handler` accessors (consistent with `conns`).
