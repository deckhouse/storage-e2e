...
// Client is the narrow set of virtualization operations the provisioner needs.
// It is defined at the point of use (accept interfaces, return structs) so that
// unit tests can substitute an in-memory fake. All methods take a context as
// the first parameter and return the concrete API objects unchanged.
...
// clientAdapter implements Client on top of the concrete virtualization client.
type clientAdapter struct {
	c *virtualization.Client
}

// NewClient adapts a *virtualization.Client to the narrow Client interface used
// by the provisioner.
