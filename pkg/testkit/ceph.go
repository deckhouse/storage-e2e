...
func EnsureCephStorageClass(ctx context.Context, kubeconfig *rest.Config, cfg CephStorageClassConfig) (string, error) {
	...
	logger.Step(8, "Creating CephStorageClass %q → StorageClass", cfg.StorageClassName)
	if err := kubernetes.CreateCephStorageClass(ctx, kubeconfig, kubernetes.CephStorageClassConfig{
		...
		Type:                      kubernetes.CephStorageClassTypeRBD,
		RBDPool:                   cfg.PoolName,
		RBDDefaultFSType:          cfg.RBDDefaultFSType,
	}); err != nil {
